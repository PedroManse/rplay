...
"#
    ...
    pub async fn download(&mut self, db: &mut DBCon, music_path: &str) -> Result<()> {
        ...
        let mut dl = deezer_downloader::downloader::Downloader::new()
            .await
            .unwrap();
        let path: PathBuf = [music_path, ".downloaded", &format!("{}.{}.mp3", self.name.replace("/", "|"), self.id)].iter().collect();
        println!("{path:?}");
        let song = dl.download_song(self.deezer_id.unwrap() as u64).await.unwrap();
...
impl Playlist {
    pub async fn new(db: &mut DBCon, name: &str) -> Result<Self> {
        todo!()
    }
    pub async fn get(db: &mut DBCon, id: i64) -> Result<Self> {
        PlaylistInfo::get(db, id).await?.into_playlist(db).await
    }
    pub async fn get_all(db: &mut DBCon) -> Result<Self> {
        todo!()
    }
    ...
    pub async fn get_tracks(&self, db: &mut DBCon) -> Result<Vec<Track>> {
        todo!()
    }
}
